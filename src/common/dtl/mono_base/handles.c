...
static void shared_init (void)
{
	...
	//
	// TODO: Because anybackup media server is only one process/multi-threads, so don't implement inter-process shared file.
	//  Otherwise, if the previous process aborted, the opened handle cannot be clean and the following process cannot
	// access these files anymore.
	//
	//_wapi_fileshare_layout = _wapi_shm_attach (WAPI_SHM_FILESHARE);
	//g_assert (_wapi_fileshare_layout != NULL);
	_wapi_fileshare_layout = g_new0 (struct _WapiFileShareLayout, 1);
