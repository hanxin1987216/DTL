				   ...
				   const gchar *name G_GNUC_UNUSED)
{
...
#ifdef DEBUG
	g_message ("%s: Creating named event [%s]", __func__, name);
#endif
	...
	if (offset == 0) {
		...
		memcpy (&namedevent_handle.sharedns.name, name, namelen);
...
gpointer OpenEvent (guint32 access G_GNUC_UNUSED, gboolean inherit G_GNUC_UNUSED, const gchar *name)
{
	gpointer handle;
	int thr_ret;
	gpointer ret = NULL;
...
#ifdef DEBUG
	g_message ("%s: Opening named event [%s]", __func__, name);
#endif
