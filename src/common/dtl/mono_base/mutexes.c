...
#include <mono_base/misc-private.h>
#include <mono_base/handles-private.h>
#include <mono_base/mono-mutex.h>
#include <mono_base/mutex-private.h>
			...
			const gchar *name)
{
...
#ifdef DEBUG
	g_message ("%s: Creating named mutex [%s]", __func__, name);
#endif
	...
	if (offset == 0) {
		...
		memcpy (&namedmutex_handle.sharedns.name, name, namelen);
...
gpointer OpenMutex (guint32 access G_GNUC_UNUSED, gboolean inherit G_GNUC_UNUSED, const gchar *name)
{
	gpointer handle;
	int thr_ret;
	gpointer ret = NULL;
...
#ifdef DEBUG
	g_message ("%s: Opening named mutex [%s]", __func__, name);
#endif
	...
	if (offset == -1) {
		...
		SetLastError (ENOENT);	/* yes, really */
		goto cleanup;
	}
